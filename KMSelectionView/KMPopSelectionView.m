...
@interface KMPopSelectionView () <UITableViewDelegate, UITableViewDataSource>

@property (nonatomic) KMPopSelectionViewType type;
...
@implementation KMPopSelectionView
...
- (instancetype)initWithFrame:(CGRect)frame type:(KMPopSelectionViewType)type titles:(NSArray *)titles options:(NSMutableDictionary *)options {
...
- (void)initSubviewsWithType:(KMPopSelectionViewType)type {
    
    if (type == KMPopSelectionViewTypeDefault) {
...
- (void)doAnimationToPoint:(CGPoint)to inView:(UIView *)view fromDirection:(KMDirection)direction {
    ...
    if (!self.transparentButton) {
        ...
        if ([view isKindOfClass:[UIScrollView class]]) {
            ...
            height = height >= view.height ? height : view.height;
        } else {
            height = view.height;
        }
        ...
        if (direction == KMDirectionUp) {
            ...
            frame = CGRectMake(0, 0, view.frame.size.width, to.y + self.height);
        } else {
...
- (void)cancelHide {
    if (self.delegate && [self.delegate respondsToSelector:@selector(popSelectionViewDidCancel:)]) {
        [self.delegate popSelectionViewDidCancel:self];
    }
    [self hide];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    KMPopSelectionCell *cell = [tableView dequeueReusableCellWithIdentifier:@"selectionCell"];
    
    if (!cell) {
        cell = [[KMPopSelectionCell alloc] initWithCellStyle:KMPopSelectionCellStyleDefault
                                             reuseIdentifier:@"selectionCell"
                                                       witdh:self.frame.size.width];
        cell.delegate = self;
        XKLog(@"options:%@", cell.delegate.options);
    }
...
//    [tableView deselectRowAtIndexPath:indexPath animated:YES];
    if (self.delegate && [self.delegate respondsToSelector:@selector(popSelectionView:didSelectIndex:)]) {
        [self.delegate popSelectionView:self didSelectIndex:indexPath.row];
    }
}
